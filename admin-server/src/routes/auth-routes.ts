 ...
 * Migrated from monolith to Admin Server, now using @agent-memory/auth library
 */
...
export function createAuthRoutes(
  ...
  router.post('/login', async (req: Request, res: Response) => {
    try {
      ...
      // Generate access token (15 minutes) using @agent-memory/auth
  ...
  router.post('/register', async (req: Request, res: Response) => {
    try {
      ...
      // Generate access token using @agent-memory/auth
      const accessToken = tokenService.generateAccessToken(user_id, tenant_id || 'default', ['user']);
  ...
  router.post('/token/refresh', async (req: Request, res: Response) => {
    try {
      ...
      // Generate new access token using @agent-memory/auth
 ...
 */
function extractDeviceInfo(req: Request): Record<string, any> {
