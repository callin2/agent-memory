...
-- 2. Generate embeddings using local Qwen3 embedding model
...
-- - Dimensions: 1024
-- - Endpoint: http://172.30.1.23:1234 (local API)
...
ALTER TABLE session_handoffs
  ADD COLUMN IF NOT EXISTS embedding vector(1024); -- Qwen3 = 1024 dimensions

COMMENT ON COLUMN session_handoffs.embedding IS
  'Vector embedding for semantic similarity search. Generated by local Qwen3 text-embedding-qwen3-embedding-0.6b model.';
...
COMMENT ON COLUMN semantic_memory.embedding IS
  'Vector embedding for semantic principle similarity search. Generated by local Qwen3 model.';
...
--    const response = await fetch('http://172.30.1.23:1234/v1/embeddings', {
--      method: 'POST',
...
--      })
--    });
--    const embedding = response.data[0].embedding; // 1024 dimensions
--
-- 2. Store embedding:
--    UPDATE session_handoffs
--    SET embedding = '[...1024 dimensions...]'
--    WHERE handoff_id = '...';
--
-- 3. Semantic search:
