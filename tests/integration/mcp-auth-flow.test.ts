...
describe('MCP Authentication Flow Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    await pool.query(
      ...
      ['evt_t1_1', 'tenant_1', 'sess_t1', 'private', 'human', 'user_t1', 'message', '{"text": "Hello from tenant 1"}']
    );

    await pool.query(
      `INSERT INTO events (event_id, tenant_id, session_id, channel, actor_type, actor_id, kind, content)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
      ['evt_t1_2', 'tenant_1', 'sess_t1', 'private', 'human', 'user_t1', 'message', '{"text": "Another message"}']
    );

    await pool.query(
      `INSERT INTO decisions (decision_id, tenant_id, status, scope, decision)
       VALUES ($1, $2, $3, $4, $5)`,
      ['dec_t1_1', 'tenant_1', 'active', 'project', 'Use TypeScript for backend']
    );
    ...
    await pool.query(
      `INSERT INTO events (event_id, tenant_id, session_id, channel, actor_type, actor_id, kind, content)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
      ['evt_t2_1', 'tenant_2', 'sess_t2', 'private', 'human', 'user_t2', 'message', '{"text": "Secret from tenant 2"}']
    );
  ...
  describe('Complete MCP Flow with JWT Authentication', () => {
    it('should authenticate, call tools, and maintain tenant isolation', async () => {
      ...
      expect(recordResponse.result?.content[0]?.text).toContain('Event recorded');
      ...
      expect(queryResponse.result?.data.every((d: any) => d.tenant_id === 'tenant_1')).toBe(true);
    });
  ...
  describe('Complete MCP Flow with API Key Authentication', () => {
    it('should authenticate with API key and call tools', async () => {
      ...
      const { apiKey } = await apiKeyService.generateAPIKey('tenant_1', 'user_t1', 'Test API Key', ['read', 'write']);
    ...
    it('should track API key usage', async () => {
      const { apiKey, keyId } = await apiKeyService.generateAPIKey('tenant_1', 'user_t1', 'Trackable Key');
  ...
  describe('Failed Authentication Followed by Successful Retry', () => {
    it('should allow retry after failed authentication', async () => {
      ...
      expect(successResponse.result?.serverInfo?.name).toBe('agent-memory-system');
  ...
  describe('Audit Logs Capture All MCP Operations', () => {
    it('should log complete authentication and tool usage flow', async () => {
      ...
      const authLogs = logs.filter((log) => log.event_type === 'auth.mcp_connection');
    ...
    it('should log failed access attempts', async () => {
      ...
      const deniedLogs = logs.filter((log) => log.event_type === 'mcp_access_denied');
  ...
  describe('Multiple Concurrent Connections', () => {
    it('should handle multiple authenticated connections independently', async () => {
      ...
      expect(response1.result?.data.every((d: any) => d.tenant_id === 'tenant_1')).toBe(true);
      expect(response2.result?.data.every((d: any) => d.tenant_id === 'tenant_2')).toBe(true);
    });
  });
  ...
  describe('Tool Schema Validation', () => {
    it('should provide correct tool schemas', async () => {
      ...
      // Check tool schemas
      const recordEventTool = tools?.find((t: any) => t.name === 'memory_record_event');
