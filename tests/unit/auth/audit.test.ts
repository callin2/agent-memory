...
describe("AuditService", () => {
  ...
  beforeEach(async () => {
    ...
    // Additional users used by query tests
    await pool.query(
      `INSERT INTO users (user_id, tenant_id, username, password_hash, roles)
       VALUES ($1, $2, $3, $4, $5)`,
      ["user_1", "test_tenant", "user1", "hash", ["user"]],
    );

    await pool.query(
      `INSERT INTO users (user_id, tenant_id, username, password_hash, roles)
       VALUES ($1, $2, $3, $4, $5)`,
      ["user_2", "test_tenant", "user2", "hash", ["user"]],
    );
  });
  ...
  describe("logAuthEvent", () => {
    it("should log authentication events", async () => {
      ...
      (mockReq as any).tenant_id = "test_tenant";
  ...
  describe("queryAuditLogs", () => {
    ...
    it("should respect offset", async () => {
      const logs1 = await auditService.queryAuditLogs("test_tenant", {
        limit: 2,
        offset: 0,
      });
      const logs2 = await auditService.queryAuditLogs("test_tenant", {
        limit: 2,
        offset: 2,
      });
  ...
  describe("getRecentUserEvents", () => {
    it("should return recent events for user", async () => {
      ...
      // Ensure referenced user exists (audit_logs may enforce FK in some schemas)
      await pool.query(
        `INSERT INTO users (user_id, tenant_id, username, password_hash, roles)
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT (user_id) DO NOTHING`,
        ["other_user", "test_tenant", "other", "hash", ["user"]],
      );
